...
for id in range(1,args.num_of_students):
    ...
    for vm in vms['vms']:
        ...
        if "Jumphost" in vm['name']:
            rdpfile = open("rdp"+str(id)+'.rdp','w')
            rdpfile.write("full address:s:"+vm['firstDns']['ip']+'\r\n')
            rdpfile.write("redirectprinters:i:0\r\n")
            rdpfile.write("desktopheight:i:900\r\n")
            rdpfile.write("desktopwidth:i:1440\r\n")
            rdpfile.close()
    #except (RuntimeError, TypeError, NameError, KeyError, ValueError):
